...
- Os dois endpoints de IA (`/api/analisar-ia` e `/api/tesouro-ipca-vision`) deixam de usar o SDK `@google/genai` com API key (`GEMINI_API_KEY`) e passam a chamar o Vertex AI REST v1 (endpoint global) com OAuth de service account (`VERTEX_SA_KEY`), via cliente próprio em `functions/api/_shared/vertex.ts` (JWT RS256 por WebCrypto, cache de token por identidade de chave, single-flight, allowlist de localização e erros HTTP com status preservado).
- Os seletores de modelo do módulo Oráculo no admin-app passam a ser honrados: `modeloAnalise` e `modeloVision` são lidos de `admin_module_configs` (`oraculo-config`) com allowlist de modelos validados e fallback para `gemini-3.1-pro-preview` (novo módulo `functions/api/_shared/oraculoModelConfig.ts`). Antes, o modelo era fixo no código.
